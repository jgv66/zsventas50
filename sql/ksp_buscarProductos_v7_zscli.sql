...
-- ult.modif 22-07-2018
-- exec ksp_buscarProductos_v7_zscli 'Ne','225','CLM',20,'10P',false,'','',0,'01','ZOP';
IF OBJECT_ID('ksp_buscarProductos_v7_zscli', 'P') IS NOT NULL  
    DROP PROCEDURE ksp_buscarProductos_v7_zscli;  
GO  
CREATE PROCEDURE ksp_buscarProductos_v7_zscli (
...
BEGIN
	...
	declare @dsctomovil varchar(10)   = '0';
	...
	-- existe descuentos definidos como dscto.movil?
	select @dsctomovil = rtrim(coalesce(NOKOCARAC,'0')) from TABCARAC WHERE KOTABLA='DSCTO_MOV' AND KOCARAC='DSCTO_MOV' ; 
    ...
    set @query +=         'L.PP01UD as precio,round((L.PP01UD-(L.PP01UD*(L.DTMA01UD+'+@dsctomovil+')/100)),0) as preciomayor,0.0 as montolinea,L.DTMA01UD as descuentomax,round(L.PP01UD*(L.DTMA01UD+'+@dsctomovil+')/100,0) as dsctovalor,';
	...
	set @query +=         @dsctomovil+' as descuentomovil ';
